  ...
  return (
    <main className="mx-auto grid max-w-[1240px] gap-5 px-4 py-8 sm:px-5 lg:grid-cols-[1fr_0.72fr] lg:px-6 lg:py-10">
      <section className="rounded-2xl border border-brand-100 bg-brand-50 p-5 sm:p-6 lg:p-7">
        <p className="text-[11px] font-semibold uppercase tracking-[0.13em] text-brand-700">Espacio privado</p>
        <h1 className="mt-2.5 max-w-xl text-3xl font-semibold tracking-[-0.025em] text-slate-900 sm:text-4xl">
          Convierte una demo navegable en una aplicación configurable.
        </h1>
        <p className="mt-3 max-w-xl text-sm leading-6 text-slate-600">
          El sitio público sirve para descubrir y presentar propuestas. En el Compositor, los usuarios autenticados configuran marca, presets, funciones, navegación y exportación.
        </p>

        <div className="mt-5 grid gap-2 sm:grid-cols-2">
          {[
            'Elegir un preset de aplicación',
            'Ajustar marca y tema',
            'Seleccionar funciones y páginas',
            'Generar el ZIP de la aplicación',
          ].map((item) => (
      ...
      <section className="self-center rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        <p className="mt-4 text-[11px] font-semibold uppercase tracking-[0.13em] text-brand-600">Compositor de aplicaciones</p>
        <h2 className="mt-1.5 text-2xl font-semibold tracking-tight text-slate-900">Inicia sesión para continuar</h2>
        <p className="mt-2 text-sm leading-5 text-slate-600">
          Esta versión usa una sesión simulada y reemplazable. La identidad de producción podrá sustituir el adaptador sin cambiar este flujo de interfaz.
        </p>
