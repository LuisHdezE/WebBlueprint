...
export function ComponentsPage() {
  ...
  return (
    <main className="mx-auto max-w-[1440px] px-4 py-8 sm:px-5 lg:px-6 lg:py-10">
      <section>
        <p className="text-[11px] font-semibold uppercase tracking-[0.13em] text-brand-600">Component gallery</p>
        <h1 className="mt-2 max-w-3xl text-4xl font-semibold tracking-[-0.035em] text-slate-950 sm:text-5xl">Real components, documented where they live.</h1>
        <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600">
          The gallery starts with the reusable pieces already powering WebBlueprint. Live examples use registry data instead of parallel demo fixtures.
        </p>
      </section>
      ...
      {sampleApplication ? (
        <section className="mt-10">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-brand-600">Live examples</p>
              <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-900">Application display</h2>
            </div>
            <p className="text-xs text-slate-500">Source data: applicationRegistry / {sampleApplication.name}</p>
          </div>
          <div className="mt-4 grid gap-4 xl:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-100 p-3 sm:p-5">
              ...
              <div className="mt-3 rounded-xl bg-white p-3 shadow-sm ring-1 ring-slate-200">
                <p className="text-xs font-semibold text-slate-700">Compact variant</p>
      ...
      <section className="mt-10">
        <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-brand-600">Governed variants</p>
        <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-900">Left navigation shells</h2>
        ...
        {componentDocumentation.map((entry) => (
          <article className="scroll-mt-24 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" id={entry.id} key={entry.id}>
            <div className="flex flex-col gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-emerald-700">{entry.maturity}</span>
                </div>
            ...
            <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
              <div>
                <h3 className="text-xs font-semibold uppercase tracking-[0.1em] text-slate-400">Props</h3>
                {entry.props.length > 0 ? (
                  <div className="mt-2 overflow-x-auto rounded-xl border border-slate-200">
                    <table className="min-w-[720px] w-full border-collapse text-left text-xs">
                      <thead className="bg-slate-50 text-slate-500">
                        <tr><th className="px-3 py-2.5 font-semibold">Prop</th><th className="px-3 py-2.5 font-semibold">Type</th><th className="px-3 py-2.5 font-semibold">Required</th><th className="px-3 py-2.5 font-semibold">Description</th></tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {entry.props.map((prop) => (
                          <tr key={prop.name}>
                            ...
                            <td className="px-3 py-3 align-top text-slate-600">{prop.required ? 'yes' : 'no'}</td>
                ...
                ) : <p className="mt-2 rounded-xl bg-slate-50 p-3 text-sm text-slate-500">No public props. This component is used as a router layout.</p>}
              </div>

              <div className="space-y-4">
                <SpecList label="Variants" values={entry.variants} />
                <SpecList label="States" values={entry.states} />
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.1em] text-slate-400">Usage</p>
  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="border-t border-slate-100 p-3">
        ...
        <p className="mt-1 text-[11px] text-slate-500">{collapsible ? 'Desktop collapse + mobile drawer' : dark ? 'Persistent dark desktop surface + mobile drawer' : 'Persistent light desktop surface + mobile drawer'}</p>
      </div>
    </article>
  );
}
